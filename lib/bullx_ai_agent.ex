defmodule BullXAIAgent do
  ...
  BullXAIAgent provides a unified interface for AI interactions, built on ReqLLM and
  ...
  Then call the facade directly:
      ...
      {:ok, response} = BullXAIAgent.generate_text("Summarize this in one sentence.")
      {:ok, json} = BullXAIAgent.generate_object("Extract fields", schema)
      {:ok, stream} = BullXAIAgent.stream_text("Stream this response")
  ...
  Register and unregister tools dynamically with running agents:
      ...
      # Register a new tool
      {:ok, agent} = BullXAIAgent.register_tool(agent_pid, MyApp.Tools.Calculator)

      # Unregister a tool by name
      {:ok, agent} = BullXAIAgent.unregister_tool(agent_pid, "calculator")

      # List registered tools
      {:ok, tools} = BullXAIAgent.list_tools(agent_pid)

      # Check if a tool is registered
      {:ok, true} = BullXAIAgent.has_tool?(agent_pid, "calculator")
  ...
  ## Examples
      ...
      {:ok, agent} = BullXAIAgent.register_tool(agent_pid, MyApp.Tools.Calculator)
      {:error, :not_a_tool} = BullXAIAgent.register_tool(agent_pid, NotATool)
  ...
  ## Examples
      ...
      # With agent struct
      tools = BullXAIAgent.list_tools(agent)
  ...
  ## Examples
      ...
      # With agent struct
      true = BullXAIAgent.has_tool?(agent, "calculator")
