...
defmodule BullXAIAgent.TRMAgent do
  @moduledoc """
  Base macro for TRM (Tiny-Recursive-Model) agents.

  Wraps `use Jido.Agent` with `BullXAIAgent.Reasoning.TRM.Strategy` wired in,
  plus standard state fields and helper functions.

  ## Usage

      defmodule MyApp.ReasoningAgent do
        use BullXAIAgent.TRMAgent,
  ...
  defmacro __using__(opts) do
    ...
    quote location: :keep do
      use Jido.Agent,
        ...
        strategy: {BullXAIAgent.Reasoning.TRM.Strategy, unquote(Macro.escape(strategy_opts))},
      ...
      def on_before_cmd(
            agent,
            {:trm_request_error, %{request_id: request_id, reason: reason, message: message}} =
              action
          ) do
