...
defmodule BullXAIAgent.ToTAgent do
  @moduledoc """
  Base macro for Tree-of-Thoughts-powered agents.

  Wraps `use Jido.Agent` with `BullXAIAgent.Reasoning.TreeOfThoughts.Strategy` wired in,
  plus standard state fields and helper functions.

  ## Usage

      defmodule MyApp.PuzzleSolver do
        use BullXAIAgent.ToTAgent,
          name: "puzzle_solver",
  ...
  defmacro __using__(opts) do
    ...
    quote location: :keep do
      use Jido.Agent,
        ...
        strategy:
          {BullXAIAgent.Reasoning.TreeOfThoughts.Strategy, unquote(Macro.escape(strategy_opts))},
        schema: unquote(base_schema_ast)

      unquote(BullXAIAgent.Agent.compatibility_overrides_ast())

      alias BullXAIAgent.Request
      alias BullXAIAgent.Reasoning.TreeOfThoughts.Result
      ...
      def on_before_cmd(
            agent,
            {:tot_request_error, %{request_id: request_id, reason: reason, message: message}} =
              action
          ) do
