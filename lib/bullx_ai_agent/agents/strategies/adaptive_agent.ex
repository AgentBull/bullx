...
defmodule BullXAIAgent.AdaptiveAgent do
  @moduledoc """
  Base macro for Adaptive strategy-powered agents.

  Wraps `use Jido.Agent` with `BullXAIAgent.Reasoning.Adaptive.Strategy` wired in,
  plus standard state fields and helper functions.

  ## Usage

      defmodule MyApp.SmartAssistant do
        use BullXAIAgent.AdaptiveAgent,
  ...
  defmacro __using__(opts) do
    ...
    quote location: :keep do
      use Jido.Agent,
        ...
        strategy:
          {BullXAIAgent.Reasoning.Adaptive.Strategy, unquote(Macro.escape(strategy_opts))},
      ...
      def on_before_cmd(
            agent,
            {:adaptive_request_error, %{request_id: request_id, reason: reason, message: message}} =
              action
          ) do
