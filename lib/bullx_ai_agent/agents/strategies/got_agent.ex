...
defmodule BullXAIAgent.GoTAgent do
  @moduledoc """
  Base macro for Graph-of-Thoughts-powered agents.

  Wraps `use Jido.Agent` with `BullXAIAgent.Reasoning.GraphOfThoughts.Strategy` wired in,
  plus standard state fields and helper functions.

  ## Usage

      defmodule MyApp.ResearchSynthesizer do
        use BullXAIAgent.GoTAgent,
  ...
  defmacro __using__(opts) do
    ...
    strategy_opts =
      ...
      |> then(fn o ->
        if aggregation_prompt,
          do: Keyword.put(o, :aggregation_prompt, aggregation_prompt),
          else: o
      end)
    ...
    quote location: :keep do
      use Jido.Agent,
        ...
        strategy:
          {BullXAIAgent.Reasoning.GraphOfThoughts.Strategy, unquote(Macro.escape(strategy_opts))},
      ...
      def on_before_cmd(
            agent,
            {:got_request_error, %{request_id: request_id, reason: reason, message: message}} =
              action
          ) do
