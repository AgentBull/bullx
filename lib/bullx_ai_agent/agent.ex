...
defmodule BullXAIAgent.Agent do
  ...
  and is accessible via `BullXAIAgent.Directive.Helpers.get_task_supervisor/1`. It is automatically
  ...
  defmacro __using__(opts) do
      ...
      case system_prompt_raw do
        ...
        other ->
          ...
          if Macro.quoted_literal?(expanded) do
            ...
            __MODULE__.normalize_system_prompt_value(
              resolved,
              __CALLER__.file,
              system_prompt_line
            )
    ...
    quote location: :keep do
      ...
      import BullXAIAgent.Agent, only: [tools_from_skills: 1]
      ...
      def on_before_cmd(
        ...
        Request.Stream.send_event(
          stream_to,
          Request.Stream.failed_event(request_id, error, reason: reason)
        )
      ...
      def on_after_cmd(agent, _action, directives) do
        ...
        should_finalize? =
          is_binary(request_id) and request_pending?(agent, request_id) and snap.done?
      ...
      defp lifecycle_signals_enabled?(agent) do
        get_in(agent.state, [:__strategy__, :config, :observability, :emit_lifecycle_signals?]) !=
          false
      end
  ...
  ## Example
      ...
      use BullXAIAgent.Agent,
        name: "weather_agent",
        tools: BullXAIAgent.Agent.tools_from_skills(@skills),
