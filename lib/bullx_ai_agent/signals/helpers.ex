defmodule BullXAIAgent.Signal.Helpers do
  @moduledoc """
  Shared helpers for signal correlation and standardized AI runtime error envelopes.

  The envelope defined here is owned by `bullx_ai_agent`. Upstream packages such as
  ...
  def normalize_error(:timeout, _fallback_type, _fallback_message, extra_details)
      when is_map(extra_details) do
  ...
  def normalize_error(reason, fallback_type, fallback_message, extra_details)
      when is_map(extra_details) do
    error_envelope(
      fallback_type,
      fallback_message,
