defmodule BullXAIAgent.Quota.Store do
  @moduledoc """
  In-process quota counters backed by ETS.
  """

  @table :bullx_ai_agent_quota_store
  @heir_name :bullx_ai_agent_quota_store_heir
  ...
  def add_usage(scope, tokens, window_ms)
  ...
  def status(scope, limits, window_ms) when is_binary(scope) and is_map(limits) do
    ...
    over_requests? =
      is_integer(max_requests) and max_requests >= 0 and usage.requests >= max_requests

    over_tokens? =
      is_integer(max_total_tokens) and max_total_tokens >= 0 and
  ...
  defp maybe_roll_window!(scope, now, window_ms) when is_integer(window_ms) and window_ms > 0 do
    case :ets.lookup(@table, scope) do
      [{^scope, started_at, _requests, _total_tokens}]
      when is_integer(started_at) and now - started_at >= window_ms ->
