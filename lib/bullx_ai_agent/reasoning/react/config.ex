defmodule BullXAIAgent.Reasoning.ReAct.Config do
  @moduledoc """
  ...
  @legacy_insecure_token_secret "bullx_ai_agent_react_default_secret_change_me"
  @ephemeral_secret_key {:bullx_ai_agent, __MODULE__, :ephemeral_token_secret}
  @ephemeral_secret_warned_key {:bullx_ai_agent, __MODULE__, :ephemeral_token_secret_warned}
  ...
  def new(opts \\ %{}) do
    ...
    resolved_model = opts_map |> get_opt(:model, @default_model) |> BullXAIAgent.resolve_model()
    ...
    tool_exec = %{
      ...
      retry_backoff_ms:
        normalize_non_neg_integer(get_opt(opts_map, :tool_retry_backoff_ms, 200), 200),
    ...
    token_secret =
      opts_map
      |> get_opt(:token_secret, BullXAIAgent.config(:react_token_secret, nil))
    ...
    attrs = %{
      ...
      tools: tools,
      request_transformer:
        normalize_request_transformer(get_opt(opts_map, :request_transformer, nil)),
  ...
  def fingerprint(%__MODULE__{} = config) do
    ...
    parts = [
      "v#{config.version}",
      BullXAIAgent.model_fingerprint_segment(config.model),
  ...
  def merge_llm_opts(%__MODULE__{} = _config, base_opts, nil) when is_list(base_opts),
    do: base_opts
  ...
  defp normalize_token_secret(secret) when is_binary(secret) and secret != "" do
    if secret == @legacy_insecure_token_secret do
      raise ArgumentError,
            "insecure ReAct token secret rejected; configure :bullx, BullXAIAgent, react_token_secret: ... or pass :token_secret explicitly"
    else
      secret
    end
  end
  ...
  defp maybe_warn_ephemeral_secret do
    unless :persistent_term.get(@ephemeral_secret_warned_key, false) do
      ...
      Logger.warning(
        "BullXAIAgent.Reasoning.ReAct using ephemeral token secret (no configured :react_token_secret); checkpoint tokens expire on VM restart"
      )
    end
  end
  ...
  defp normalize_llm_opts(value, provider_opt_keys_by_string) when is_map(value) do
    ...
    |> Enum.map(fn {key, entry_value} ->
      ...
      normalized_value =
        normalize_llm_opt_value(normalized_key, entry_value, provider_opt_keys_by_string)
  ...
  defp provider_opt_keys_by_string(model_spec), do: BullXAIAgent.provider_opt_keys(model_spec)
