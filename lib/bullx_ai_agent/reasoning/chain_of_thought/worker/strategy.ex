defmodule BullXAIAgent.Reasoning.ChainOfThought.Worker.Strategy do
  @moduledoc false
  ...
  alias BullXAIAgent.Runtime.Event
  ...
  defp start_run(
         agent,
         %{request_id: request_id, run_id: run_id, prompt: prompt, config: config_input} = params
       )
    ...
    if state[:status] == :running and is_binary(state[:active_request_id]) do
      ...
      case start_task(
             fn -> run_stream(worker_pid, request_id, run_id, prompt, config) end,
             task_supervisor
           ) do
        {:ok, runtime_task} ->
          new_state =
  ...
  defp run_stream(worker_pid, request_id, run_id, prompt, config) do
    ...
    done_signal =
      Jido.Signal.new!("ai.cot.worker.runtime.done", %{request_id: request_id}, source: @source)
  ...
  defp maybe_emit_delta(_worker_pid, _request_id, _run_id, _llm_call_id, _chunk, _config, seq),
    do: {seq, nil}
