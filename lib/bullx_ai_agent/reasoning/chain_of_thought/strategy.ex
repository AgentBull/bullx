defmodule BullXAIAgent.Reasoning.ChainOfThought.Strategy do
  @moduledoc """
  ...
  alias BullXAIAgent.Reasoning.ChainOfThought.Machine
  ...
  defp apply_worker_event(state, event) do
    ...
    case kind do
      :request_started ->
        ...
        signal =
          Signal.RequestStarted.new!(%{request_id: request_id, query: prompt, run_id: request_id})
      ...
      :llm_completed ->
        ...
        usage_signal =
          maybe_usage_signal(call_id || "", config_model(state), usage, request_id, run_id)
  ...
  defp maybe_usage_signal(_call_id, _model, usage, _request_id, _run_id) when usage in [%{}, nil],
    do: nil
  ...
  defp emit_runtime_telemetry(state, kind, request_id, run_id, llm_call_id, data) do
    ...
    metadata = %{
      ...
      error_type:
        if(kind == :request_failed, do: infer_error_type(event_field(data, :error)), else: nil)
    }
  ...
  defp busy?(state, config) do
    config[:request_policy] == :reject and state[:status] == :reasoning and
    ...
    if is_pid(state[:cot_worker_pid]) and Process.alive?(state[:cot_worker_pid]) do
      ...
      spawn_directive =
        AgentDirective.spawn_agent(
          BullXAIAgent.Reasoning.ChainOfThought.Worker.Agent,
          @worker_tag
        )
      ...
      system_prompt:
        normalize_system_prompt_opt(
          Map.get(
            agent.state,
            :system_prompt,
            Keyword.get(opts, :system_prompt, Machine.default_system_prompt())
          )
        ),
      model: resolved_model,
      request_policy:
        Map.get(agent.state, :request_policy, Keyword.get(opts, :request_policy, :reject)),
      llm_timeout_ms: Map.get(agent.state, :llm_timeout_ms, Keyword.get(opts, :llm_timeout_ms)),
      runtime_task_supervisor:
        Map.get(
          agent.state,
          :runtime_task_supervisor,
          Keyword.get(opts, :runtime_task_supervisor)
        ),
      runtime_adapter: true,
      observability:
        Map.merge(
          ...
          Map.get(agent.state, :observability, opts |> Keyword.get(:observability, %{}))
          |> normalize_map_opt()
        )
    }
  end
  ...
  defp normalize_system_prompt_opt(prompt) when prompt in [nil, false, ""],
    do: Machine.default_system_prompt()
