defmodule BullXAIAgent.Reasoning.Helpers do
  @moduledoc """
  Helper functions for creating StateOps in BullXAIAgent strategies.
  ...
  * `DeletePath` - Delete value at nested path
  ...
  @spec maybe_execute_action_instruction(Agent.t(), Jido.Instruction.t(), strategy_ctx()) ::
          {Agent.t(), [struct()]} | :noop
  def maybe_execute_action_instruction(
        %Agent{} = agent,
  ...
  ## Examples
      ...
      [:config, :tools]
  """
  @spec update_tools_config([module()], %{String.t() => module()}, [map()]) :: [
          StateOp.SetPath.t()
        ]
  ...
  defp run_instruction(%Jido.Instruction{} = instruction),
    do: :erlang.apply(Jido.Exec, :run, [instruction])
