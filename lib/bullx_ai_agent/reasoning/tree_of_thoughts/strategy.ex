defmodule BullXAIAgent.Reasoning.TreeOfThoughts.Strategy do
  @moduledoc """
  ...
  This strategy uses a pure state machine (`BullXAIAgent.Reasoning.TreeOfThoughts.Machine`) for
  ...
  Configure via strategy options when defining your agent:
      ...
      use Jido.Agent,
        name: "my_tot_agent",
        strategy: {
          BullXAIAgent.Reasoning.TreeOfThoughts.Strategy,
  ...
  - `:model` (optional) - Model alias or direct model spec, defaults to :fast (resolved via BullXAIAgent.resolve_model/1)
  ...
  alias BullXAIAgent.Reasoning.TreeOfThoughts.Machine
  ...
  defp process_llm_result(agent, %{call_id: call_id, result: result}, config, _ctx)
  ...
  defp process_tool_result(agent, %{call_id: tool_call_id} = params, config)
  ...
  defp tooling_enabled?(config) do
    is_list(config[:reqllm_tools]) and config[:reqllm_tools] != [] and
      is_map(config[:actions_by_name])
  end
    ...
    cond do
      ...
      round >= max_round_trips ->
        apply_machine_update(
          agent,
          {:llm_result, llm_call_id, {:error, :max_tool_round_trips_reached}},
          config
        )
  ...
  defp has_pending_tool_round?(state, llm_call_id) do
    state[:pending_tool_call_id] == llm_call_id and
  ...
  defp apply_pending_tool_effects(agent, state, policy) do
    ...
    |> Enum.reduce({agent, []}, fn call_id, {acc_agent, acc_directives_rev} ->
      ...
      {updated_agent, directives, _stats, _filtered_result} =
        Effects.apply_result(acc_agent, result, policy)

      {updated_agent, Enum.reverse(directives, acc_directives_rev)}
    end)
    |> then(fn {updated_agent, directives_rev} ->
      {updated_agent, Enum.reverse(directives_rev)}
    end)
  end
    ...
    {lifted, call_aliases} =
      Enum.reduce(directives, {[], %{}}, fn
        {:generate_thoughts, id, conversation, _count}, {acc_dirs, aliases} ->
          directive =
            build_llm_directive(id, model, conversation, config, llm_tools_for_request(config))
        ...
        {:evaluate_thoughts, id, thoughts}, {acc_dirs, aliases} ->
          ...
          directive =
            build_llm_directive(id, model, context, config, llm_tools_for_request(config))
  ...
  defp build_config(agent, ctx) do
    ...
    %{
      ...
      branching_factor:
        Map.get(agent.state, :branching_factor, Keyword.get(opts, :branching_factor, 3)),
      max_depth: Map.get(agent.state, :max_depth, Keyword.get(opts, :max_depth, 3)),
      traversal_strategy:
        Map.get(
          agent.state,
          :traversal_strategy,
          Keyword.get(opts, :traversal_strategy, :best_first)
        ),
      generation_prompt:
        Map.get(
          agent.state,
      ...
      min_depth:
        Map.get(agent.state, :min_depth, Keyword.get(opts, :min_depth, @default_min_depth)),
      max_nodes:
        Map.get(agent.state, :max_nodes, Keyword.get(opts, :max_nodes, @default_max_nodes)),
      max_duration_ms:
        Map.get(agent.state, :max_duration_ms, Keyword.get(opts, :max_duration_ms)),
      ...
      max_parse_retries:
        Map.get(
          agent.state,
          :max_parse_retries,
          Keyword.get(opts, :max_parse_retries, @default_max_parse_retries)
        ),
      ...
      tool_retry_backoff_ms:
        Keyword.get(opts, :tool_retry_backoff_ms, @default_tool_retry_backoff_ms),
      max_tool_round_trips:
        Keyword.get(opts, :max_tool_round_trips, @default_max_tool_round_trips),
