defmodule BullXAIAgent.Reasoning.TRM.Strategy do
  @moduledoc """
  ...
  This strategy uses a pure state machine (`BullXAIAgent.Reasoning.TRM.Machine`) for all state
  ...
  Configure via strategy options when defining your agent:
      ...
      use Jido.Agent,
        name: "my_trm_agent",
        strategy: {
          BullXAIAgent.Reasoning.TRM.Strategy,
  ...
  alias BullXAIAgent.Reasoning.RequestLifecycle
  alias BullXAIAgent.Reasoning.TRM.Machine
  alias BullXAIAgent.Reasoning.TRM.Reasoning
  alias BullXAIAgent.Reasoning.TRM.Supervision
  ...
  # Length validation is handled by BullXAIAgent.Reasoning.TRM.Helpers.sanitize_user_input/2
    ...
    %{
      ...
      act_threshold:
        Map.get(
          agent.state,
          :act_threshold,
          Keyword.get(opts, :act_threshold, @default_act_threshold)
        )
    }
  end
  ...
  defp to_machine_msg(@start, params, _status) do
    ...
    request_id =
      Map.get(params, :request_id) || Map.get(params, "request_id") || Machine.generate_call_id()
  ...
  defp merge_runtime_state(new_state, previous_state, {:start, _prompt, request_id}) do
    RequestLifecycle.put_active_request_id(previous_state, new_state, request_id, [
      :reasoning,
      :supervising,
      :improving
    ])
  ...
  defp merge_runtime_state(new_state, previous_state, _msg) do
    RequestLifecycle.put_active_request_id(previous_state, new_state, nil, [
      :reasoning,
      :supervising,
      :improving
    ])
  ...
  defp emit_request_lifecycle(previous_state, new_state, {:start, prompt, request_id}) do
    if previous_state[:status] == :idle and new_state[:status] == :reasoning do
      RequestLifecycle.emit_started(:trm, new_state, request_id, prompt,
  ...
  defp infer_msg_error_type({_phase, _call_id, {:error, reason, _effects}}) when is_atom(reason),
    do: reason
  ...
  # A future refactoring could extract these into a shared BullXAIAgent.Strategy.Helpers
  ...
  the Reasoning module. See `BullXAIAgent.Reasoning.TRM.Reasoning` for details.
  """
  ...
  This is provided for reference - prompts are managed internally by
  the Supervision module. See `BullXAIAgent.Reasoning.TRM.Supervision` for details.
  """
