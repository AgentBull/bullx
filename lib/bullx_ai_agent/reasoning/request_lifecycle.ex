defmodule BullXAIAgent.Reasoning.RequestLifecycle do
  @moduledoc """
  ...
  def emit_started(strategy, state, request_id, query, opts \\ [])
      when is_atom(strategy) and is_map(state) and is_binary(request_id) and is_binary(query) and
             is_list(opts) do
    signal =
  ...
  def emit_terminal(strategy, previous_state, new_state, opts \\ [])
    ...
    request_id =
      Keyword.get(
        opts,
        :request_id,
        new_state[:active_request_id] || previous_state[:active_request_id]
      )

    cond do
      not completed_status?(previous_state[:status]) and completed_status?(new_state[:status]) and
      ...
      not error_status?(previous_state[:status]) and error_status?(new_state[:status]) and
          is_binary(request_id) ->
        signal =
          Signal.RequestFailed.new!(%{
            request_id: request_id,
            error: new_state[:result],
            run_id: request_id
          })
  ...
  def emit_request_telemetry(strategy, state, event, request_id, opts \\ [])
    ...
    metadata = %{
      ...
      tool_call_id: nil,
      tool_name: nil,
      model: if(model, do: BullXAIAgent.model_label(model), else: nil),
      origin: :worker_runtime,
      operation: operation,
      strategy: strategy,
      termination_reason: telemetry_termination_reason(event, state),
      error_type:
        if(event == :failed, do: error_type || infer_error_type(state[:result]), else: nil)
    }
