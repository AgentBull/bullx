defmodule BullXAIAgent.Reasoning.Adaptive.Strategy do
  @moduledoc """
  ...
  Configure via strategy options when defining your agent:
      ...
      use Jido.Agent,
        ...
        strategy: {
          BullXAIAgent.Reasoning.Adaptive.Strategy,
  ...
  alias BullXAIAgent.Reasoning.AlgorithmOfThoughts.Strategy, as: AlgorithmOfThoughts
  alias BullXAIAgent.Reasoning.ChainOfDraft.Strategy, as: ChainOfDraft
  alias BullXAIAgent.Reasoning.Helpers
  alias BullXAIAgent.Reasoning.ChainOfThought.Strategy, as: ChainOfThought
  alias BullXAIAgent.Reasoning.GraphOfThoughts.Strategy, as: GraphOfThoughts
  alias BullXAIAgent.Reasoning.ReAct.Strategy, as: ReAct
  alias BullXAIAgent.Reasoning.TreeOfThoughts.Strategy, as: TreeOfThoughts
  alias BullXAIAgent.Reasoning.TRM.Strategy, as: TRM
    ...
    %{
      ...
      default_strategy:
        Map.get(
          agent.state,
          :default_strategy,
          Keyword.get(opts, :default_strategy, @default_strategy)
        ),
      ...
      complexity_thresholds:
        Map.get(
          agent.state,
          :complexity_thresholds,
          Keyword.get(opts, :complexity_thresholds, @default_thresholds)
        ),
  ...
  defp handle_initial_instruction(agent, instructions, ctx, state) do
    ...
    case start_instr do
      ...
      %{params: params} ->
        ...
        case Map.fetch(@strategy_modules, strategy_type) do
          {:ok, strategy_module} ->
  ...
  defp map_instructions(instructions, strategy_type) do
    Enum.map(instructions, fn instr ->
      ...
      mapped_params =
        map_params_for_strategy(instr_map.params || %{}, strategy_type, instr_map.action)
