defmodule BullXAIAgent.Reasoning.AgenticLoop.Strategy do
  @moduledoc """
  AgenticLoop strategy delegated to an internal per-parent worker agent.
  ...
  1. Parent receives `"ai.agentic_loop.query"` and prepares runtime config/context.
  2. Parent lazily spawns internal worker on first request (if needed).
  3. Parent emits `"ai.agentic_loop.worker.start"` to worker.
  4. Worker streams `BullXAIAgent.Reasoning.AgenticLoop` events and emits `"ai.agentic_loop.worker.event"` to parent.
  5. Parent applies runtime events to parent state and emits external lifecycle/LLM/tool signals.

  ## Worker Lifecycle

  - Child tag is fixed as `:agentic_loop_worker`.
  ...
  alias BullXAIAgent.Directive
  alias BullXAIAgent.Effects
  alias BullXAIAgent.Request.Stream, as: RequestStream
  alias BullXAIAgent.Reasoning.AgenticLoop.PendingInput
  alias BullXAIAgent.Reasoning.AgenticLoop.RequestTransformer
  alias BullXAIAgent.Reasoning.AgenticLoop.State, as: AgenticLoopState
  alias BullXAIAgent.Reasoning.AgenticLoop.Config, as: AgenticLoopRuntimeConfig
  alias BullXAIAgent.Reasoning.AgenticLoop.ToolSelection
  alias BullXAIAgent.Signal
  alias BullXAIAgent.Signal.Helpers, as: SignalHelpers
  alias BullXAIAgent.Reasoning.Helpers
  alias BullXAIAgent.Context, as: AIContext
  alias BullXAIAgent.ToolAdapter
  ...
  @worker_tag :agentic_loop_worker
  @source "/ai/agentic_loop/strategy"
  @reqllm_generation_opt_keys_by_string ReqLLM.Provider.Options.all_generation_keys()
                                        |> Enum.map(&{Atom.to_string(&1), &1})
                                        |> Map.new()

  @default_system_prompt """
  You are a helpful AI assistant using the AgenticLoop (Reason-Act) pattern.
  ...
  @start :ai_agentic_loop_start
  @llm_result :ai_agentic_loop_llm_result
  @tool_result :ai_agentic_loop_tool_result
  @llm_partial :ai_agentic_loop_llm_partial
  @cancel :ai_agentic_loop_cancel
  @steer :ai_agentic_loop_steer
  @inject :ai_agentic_loop_inject
  @request_error :ai_agentic_loop_request_error
  @register_tool :ai_agentic_loop_register_tool
  @unregister_tool :ai_agentic_loop_unregister_tool
  @set_tool_context :ai_agentic_loop_set_tool_context
  @set_system_prompt :ai_agentic_loop_set_system_prompt
  @context_modify :ai_agentic_loop_context_modify
  @runtime_event :ai_agentic_loop_runtime_event
  @worker_event :ai_agentic_loop_worker_event
  @worker_child_started :ai_agentic_loop_worker_child_started
  @worker_child_exit :ai_agentic_loop_worker_child_exit

  @doc "Returns the action atom for starting a AgenticLoop conversation."
  @spec start_action() :: :ai_agentic_loop_start
  def start_action, do: @start

  @doc "Returns the legacy action atom for handling LLM results (no-op in delegated mode)."
  @spec llm_result_action() :: :ai_agentic_loop_llm_result
  def llm_result_action, do: @llm_result

  @doc "Returns the action atom for registering a tool dynamically."
  @spec register_tool_action() :: :ai_agentic_loop_register_tool
  def register_tool_action, do: @register_tool

  @doc "Returns the action atom for unregistering a tool."
  @spec unregister_tool_action() :: :ai_agentic_loop_unregister_tool
  ...
  @spec tool_result_action() :: :ai_agentic_loop_tool_result
  ...
  @spec llm_partial_action() :: :ai_agentic_loop_llm_partial
  def llm_partial_action, do: @llm_partial

  @doc "Returns the action atom for request cancellation."
  @spec cancel_action() :: :ai_agentic_loop_cancel
  def cancel_action, do: @cancel

  @doc "Returns the action atom for steering an active AgenticLoop request."
  @spec steer_action() :: :ai_agentic_loop_steer
  def steer_action, do: @steer

  @doc "Returns the action atom for injecting user-style input into an active AgenticLoop request."
  @spec inject_action() :: :ai_agentic_loop_inject
  def inject_action, do: @inject

  @doc "Returns the action atom for handling request rejections."
  @spec request_error_action() :: :ai_agentic_loop_request_error
  def request_error_action, do: @request_error

  @doc "Returns the action atom for updating tool context."
  @spec set_tool_context_action() :: :ai_agentic_loop_set_tool_context
  def set_tool_context_action, do: @set_tool_context

  @doc "Returns the action atom for updating the base system prompt."
  @spec set_system_prompt_action() :: :ai_agentic_loop_set_system_prompt
  def set_system_prompt_action, do: @set_system_prompt

  @doc "Returns the canonical action atom for context lifecycle operations."
  @spec context_modify_action() :: :ai_agentic_loop_context_modify
  ...
  @spec runtime_event_action() :: :ai_agentic_loop_runtime_event
  ...
  @action_specs %{
    @start => %{
      ...
      doc: "Start a delegated AgenticLoop conversation with a user query",
      name: "ai.agentic_loop.start"
    },
    @cancel => %{
      schema:
        Zoi.object(%{
      ...
      doc: "Cancel an in-flight AgenticLoop request",
      name: "ai.agentic_loop.cancel"
    },
    @steer => %{
      schema:
        Zoi.object(%{
      ...
      doc: "Steer an active delegated AgenticLoop request with additional user input",
      name: "ai.agentic_loop.steer"
    },
    @inject => %{
      schema:
        Zoi.object(%{
      ...
      doc: "Inject user-style input into an active delegated AgenticLoop request",
      name: "ai.agentic_loop.inject"
    },
    @request_error => %{
      schema:
        Zoi.object(%{
    ...
    @context_modify => %{
      ...
      name: "ai.agentic_loop.context.modify"
    },
    @worker_event => %{
      schema: Zoi.object(%{request_id: Zoi.string(), event: Zoi.map()}),
      doc: "Handle delegated AgenticLoop runtime event envelopes",
      name: "ai.agentic_loop.worker.event"
    },
    ...
    @llm_result => %{
      ...
      doc: "Legacy no-op in delegated AgenticLoop mode",
      name: "ai.agentic_loop.llm_result"
    },
    @tool_result => %{
      schema: Zoi.object(%{call_id: Zoi.string(), tool_name: Zoi.string(), result: Zoi.any()}),
      doc: "Legacy no-op in delegated AgenticLoop mode",
      name: "ai.agentic_loop.tool_result"
    },
    @llm_partial => %{
      schema:
        Zoi.object(%{
      ...
      doc: "Legacy no-op in delegated AgenticLoop mode",
      name: "ai.agentic_loop.llm_partial"
    },
    @runtime_event => %{
      schema: Zoi.object(%{request_id: Zoi.string(), event: Zoi.map()}),
      doc: "Legacy no-op in delegated AgenticLoop mode",
      name: "ai.agentic_loop.runtime_event"
    }
  }
  ...
  def signal_routes(_ctx) do
    [
      {"ai.agentic_loop.query", {:strategy_cmd, @start}},
      {"ai.agentic_loop.cancel", {:strategy_cmd, @cancel}},
      {"ai.agentic_loop.steer", {:strategy_cmd, @steer}},
      {"ai.agentic_loop.inject", {:strategy_cmd, @inject}},
      {"ai.request.error", {:strategy_cmd, @request_error}},
      {"ai.agentic_loop.register_tool", {:strategy_cmd, @register_tool}},
      {"ai.agentic_loop.unregister_tool", {:strategy_cmd, @unregister_tool}},
      {"ai.agentic_loop.set_tool_context", {:strategy_cmd, @set_tool_context}},
      {"ai.agentic_loop.set_system_prompt", {:strategy_cmd, @set_system_prompt}},
      {"ai.agentic_loop.context.modify", {:strategy_cmd, @context_modify}},
      {"ai.agentic_loop.worker.event", {:strategy_cmd, @worker_event}},
  ...
  defp build_snapshot_details(state, config) do
    ...
    %{
      ...
      worker_pid: state[:agentic_loop_worker_pid],
      worker_status: state[:agentic_loop_worker_status],
  ...
  def init(%Agent{} = agent, ctx) do
    ...
    state =
      %{
        ...
        request_traces: %{},
        agentic_loop_worker_pid: nil,
        agentic_loop_worker_status: :missing,
  ...
  defp process_cancel(agent, params) do
    ...
    should_cancel? =
      is_binary(request_id) and request_id == state[:active_request_id] and
        is_pid(state[:agentic_loop_worker_pid]) and
        Process.alive?(state[:agentic_loop_worker_pid])
    ...
    case Map.fetch(state, :context) do
      ...
      {:ok, source_context} ->
        case AIContext.coerce(source_context) do
          ...
          :error ->
            raise ArgumentError,
                  "invalid initial_state[:context]; expected BullXAIAgent.Context"
        end

      :error ->
        if legacy_thread_context?(Map.get(state, :thread)) do
          raise ArgumentError,
                "initial_state[:thread] is no longer supported for AI context; use initial_state[:context] with BullXAIAgent.Context"
        else
          AIContext.new(system_prompt: config[:system_prompt])
        end
    end
  ...
  defp process_worker_child_started(agent, %{tag: tag, pid: pid}) when is_pid(pid) do
    ...
    if agentic_loop_worker_tag?(tag) do
      pending = state[:pending_worker_start]

      base_state =
        state
        |> Map.put(:agentic_loop_worker_pid, pid)
        |> Map.put(:agentic_loop_worker_status, :ready)
      ...
      if is_map(pending) do
        ...
        new_state =
          ...
          |> Map.put(:agentic_loop_worker_status, :running)
  ...
  defp process_worker_child_exit(agent, %{tag: tag, pid: pid, reason: reason}) do
    ...
    if agentic_loop_worker_tag?(tag) do
      tracked? = worker_pid_matches?(state[:agentic_loop_worker_pid], pid)
      ...
      if tracked? do
        ...
        base_state =
          state
          |> Map.put(:agentic_loop_worker_pid, nil)
          |> Map.put(:agentic_loop_worker_status, :missing)
          |> Map.put(:pending_worker_start, nil)

        if is_binary(request_id) and state[:status] in [:awaiting_llm, :awaiting_tool] do
          error = {:agentic_loop_worker_exit, reason}
          ...
          RequestStream.send_event(
            stream_to,
            RequestStream.failed_event(request_id, error, reason: :agentic_loop_worker_exit)
          )
    ...
    case kind do
      ...
      :llm_completed ->
        ...
        updated =
          ...
          |> Map.put(
            :status,
            if(turn_type == :tool_calls, do: :awaiting_tool, else: :awaiting_llm)
          )
        ...
        usage_signal =
          maybe_usage_signal(call_id, config_model(state), usage, request_id, run_id, iteration)
  ...
  defp maybe_usage_signal(_call_id, _model, usage, _request_id, _run_id, _iteration)
       when usage in [%{}, nil], do: nil
  ...
  defp maybe_usage_signal(call_id, model, usage, request_id, run_id, iteration) do
    ...
    Signal.Usage.new!(%{
      call_id: call_id,
      model: BullXAIAgent.model_label(model),
  ...
  defp runtime_config_from_strategy(config, opts) do
    ...
    AgenticLoopRuntimeConfig.new(runtime_opts)
  end
  ...
  defp ensure_worker_start(state, worker_start_payload) do
    if is_pid(state[:agentic_loop_worker_pid]) and Process.alive?(state[:agentic_loop_worker_pid]) do
      directive =
        AgentDirective.emit_to_pid(
      ...
      spawn_directive =
        AgentDirective.spawn_agent(BullXAIAgent.Reasoning.AgenticLoop.Worker.Agent, @worker_tag)

      new_state =
        state
        |> Map.put(:agentic_loop_worker_pid, nil)
        |> Map.put(:agentic_loop_worker_status, :starting)
  ...
  defp worker_start_signal(payload) do
    Jido.Signal.new!("ai.agentic_loop.worker.start", payload, source: @source)
  end

  defp worker_cancel_signal(request_id, reason) do
    Jido.Signal.new!("ai.agentic_loop.worker.cancel", %{request_id: request_id, reason: reason},
      source: @source
    )
  end

  defp agentic_loop_worker_tag?(tag), do: tag == @worker_tag or tag == Atom.to_string(@worker_tag)
  ...
  defp maybe_mark_worker_ready(state, kind)
       when kind in [:request_completed, :request_failed, :request_cancelled] do
    Map.put(state, :agentic_loop_worker_status, :ready)
  end
  ...
  defp runtime_state_from_context(%AIContext{} = context, query, request_id, run_id)
    ...
    AgenticLoopState.new(query, context.system_prompt, request_id: request_id, run_id: run_id)
    |> Map.put(:context, context)
  end
  ...
  defp build_config(agent, ctx) do
    ...
    tools_modules =
      case Keyword.fetch(opts, :tools) do
        ...
        :error ->
          raise ArgumentError,
                "BullXAIAgent.Reasoning.AgenticLoop.Strategy requires :tools option (list of Jido.Action modules)"
      end
  ...
  defp resolve_model_spec(model), do: BullXAIAgent.resolve_model(model)

  defp validate_request_policy!(:reject), do: :reject

  defp validate_request_policy!(other) do
    raise ArgumentError,
          "unsupported request_policy #{inspect(other)} for AgenticLoop; supported values: [:reject]"
  end
  ...
  defp normalize_tool_result(result),
    do: SignalHelpers.normalize_result(result, :tool_error, "Tool execution failed")
  ...
  defp emit_runtime_telemetry(
    ...
    metadata =
      %{
        ...
        model: BullXAIAgent.model_label(config_model(state)),
        origin: :worker_runtime,
    ...
    case kind do
      ...
      :llm_delta ->
        Observe.emit(obs_cfg, Observe.llm(:delta), measurements, metadata,
  ...
  defp emit_tool_completed_telemetry(
         obs_cfg,
         metadata,
         measurements,
    ...
    Observe.emit(
      ...
      Observe.tool(:error),
      measurements,
      Map.put(metadata, :termination_reason, :error)
    )
  end
  ...
  defp telemetry_termination_reason(:request_completed, data),
    do: event_field(data, :termination_reason, :complete)
  ...
  defp telemetry_termination_reason(:tool_completed, data),
    do:
      if(match?({:ok, _, _}, normalize_tool_result(event_field(data, :result))),
        do: :complete,
        else: :error
      )
  ...
  defp telemetry_error_type(:request_failed, data),
    do: infer_error_type(event_field(data, :error))

  defp telemetry_error_type(:tool_completed, data),
    do: infer_error_type(event_field(data, :result))
  ...
  defp provider_opt_keys_by_string(model_spec),
    do: BullXAIAgent.provider_opt_keys(model_spec)
