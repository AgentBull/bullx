defmodule BullXAIAgent.Reasoning.AgenticLoop.Actions.Start do
  @moduledoc """
  Start a AgenticLoop runtime execution and return an event stream.
  """

  use Jido.Action,
    name: "agentic_loop_start",
    description: "Start a Task-based AgenticLoop runtime stream",
    category: "ai",
    tags: ["agentic_loop", "runtime", "streaming"],
    vsn: "1.0.0",
    schema:
      Zoi.object(%{
        query: Zoi.string(description: "User query for AgenticLoop execution"),
  ...
  alias BullXAIAgent.Validation

  @impl Jido.Action
  def run(params, context) do
    with {:ok, _} <-
           Validation.validate_string(params[:query], max_length: Validation.max_input_length()) do
      ...
      events =
        AgenticLoop.stream(
          params[:query],
          config,
          Keyword.merge(opts, request_id: request_id, run_id: run_id)
        )
