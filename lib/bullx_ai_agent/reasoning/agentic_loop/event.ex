defmodule BullXAIAgent.Reasoning.AgenticLoop.Event do
  @moduledoc """
  Compatibility wrapper around `BullXAIAgent.Runtime.Event`.

  AgenticLoop still emits `:input_injected` as a runtime-only compatibility event even
  ...
  def new(attrs) when is_map(attrs) do
    ...
    case Zoi.parse(RuntimeEvent.schema(), attrs) do
      ...
      {:error, errors} ->
        raise ArgumentError, "invalid AgenticLoop event: #{inspect(errors)}"
    end
  end
  ...
  defp validate_kind!(%RuntimeEvent{kind: kind}) do
    raise ArgumentError,
          "invalid AgenticLoop event kind: #{inspect(kind)}; expected one of #{inspect(@kind_values)}"
  end
end
