defmodule BullXAIAgent.Reasoning.AgenticLoop.Worker.Strategy do
  @moduledoc false
  ...
  alias BullXAIAgent.Reasoning.AgenticLoop.{Config, Event, Runner, Signal}

  @start :agentic_loop_worker_start
  @cancel :agentic_loop_worker_cancel
  @runtime_event :agentic_loop_worker_runtime_event
  @runtime_done :agentic_loop_worker_runtime_done
  @runtime_failed :agentic_loop_worker_runtime_failed

  @source "/ai/agentic_loop/worker"
  ...
  @action_specs %{
    @start => %{
      ...
      doc: "Start a delegated AgenticLoop runtime run",
      name: "ai.agentic_loop.worker.start"
    },
    @cancel => %{
      schema:
        Zoi.object(%{
      ...
      doc: "Cancel an active delegated AgenticLoop runtime run",
      name: "ai.agentic_loop.worker.cancel"
    },
    @runtime_event => %{
      schema:
        Zoi.object(%{
      ...
      name: "ai.agentic_loop.worker.runtime.event"
    },
    @runtime_done => %{
      schema:
        Zoi.object(%{
      ...
      name: "ai.agentic_loop.worker.runtime.done"
    },
    @runtime_failed => %{
      schema:
        Zoi.object(%{
      ...
      name: "ai.agentic_loop.worker.runtime.failed"
    }
  }
  ...
  def signal_routes(_ctx) do
    [
      {"ai.agentic_loop.worker.start", {:strategy_cmd, @start}},
      {"ai.agentic_loop.worker.cancel", {:strategy_cmd, @cancel}},
      {"ai.agentic_loop.worker.runtime.event", {:strategy_cmd, @runtime_event}},
      {"ai.agentic_loop.worker.runtime.done", {:strategy_cmd, @runtime_done}},
      {"ai.agentic_loop.worker.runtime.failed", {:strategy_cmd, @runtime_failed}}
    ]
  end
  ...
  defp start_run(
    ...
    if state[:status] == :running and is_binary(state[:active_request_id]) do
      ...
      case start_task(
             fn -> run_stream(worker_pid, request_id, query, config, stream_opts) end,
             task_supervisor
           ) do
        {:ok, runtime_task} ->
          new_state =
  ...
  defp cancel_run(agent, %{request_id: request_id, reason: reason})
    ...
    if should_cancel? do
      send(state[:runtime_task], {:agentic_loop_stream_cancel, reason})
    end
  ...
  defp run_stream(worker_pid, request_id, query, config, stream_opts) do
    ...
    |> Enum.each(fn event ->
      signal =
        Jido.Signal.new!(
          "ai.agentic_loop.worker.runtime.event",
          %{
    ...
    done_signal =
      Jido.Signal.new!("ai.agentic_loop.worker.runtime.done", %{request_id: request_id},
        source: @source
      )
    ...
    error ->
      ...
      fail_signal =
        Jido.Signal.new!(
          "ai.agentic_loop.worker.runtime.failed",
          %{
            request_id: request_id,
