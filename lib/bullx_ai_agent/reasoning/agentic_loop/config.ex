defmodule BullXAIAgent.Reasoning.AgenticLoop.Config do
  @moduledoc """
  Canonical configuration for the Task-based AgenticLoop runtime.
  """
  ...
  def new(opts \\ %{}) do
    ...
    case Zoi.parse(@schema, attrs) do
      {:ok, config} -> config
      {:error, errors} -> raise ArgumentError, "invalid AgenticLoop config: #{inspect(errors)}"
    end
  end

  @doc """
  Returns a stable config fingerprint for runtime events.
  """
  ...
  defp normalize_request_transformer(request_transformer) do
    case RequestTransformer.validate(request_transformer) do
      ...
      {:error, {:request_transformer_not_loaded, module}} ->
        raise ArgumentError,
              "invalid AgenticLoop request_transformer: module #{inspect(module)} is not loaded"

      {:error, {:request_transformer_missing_callback, module}} ->
        raise ArgumentError,
              "invalid AgenticLoop request_transformer #{inspect(module)}: expected transform_request/4 callback"

      {:error, :invalid_request_transformer} ->
        raise ArgumentError,
              "invalid AgenticLoop request_transformer: expected a module implementing transform_request/4"
    end
  end
