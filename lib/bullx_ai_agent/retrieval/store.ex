defmodule BullXAIAgent.Retrieval.Store do
  @moduledoc """
  In-process retrieval memory store backed by ETS.
  """

  @table :bullx_ai_agent_retrieval_store
  @heir_name :bullx_ai_agent_retrieval_store_heir
  ...
  def upsert(namespace, memory) when is_binary(namespace) and is_map(memory) do
    ...
    id =
      to_string(Map.get(memory, :id) || Map.get(memory, "id") || "mem_#{Jido.Util.generate_id()}")
