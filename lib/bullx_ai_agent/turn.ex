defmodule BullXAIAgent.Turn do
  @moduledoc """
  ...
  alias BullXAIAgent.{Effects, Observe, ToolAdapter}
  ...
  def from_response(%ReqLLM.Response{} = response, opts) do
    ...
    %__MODULE__{
      ...
      reasoning_details:
        normalize_reasoning_details(Map.get(response.message || %{}, :reasoning_details)),
  ...
  def format_tool_result_content(other) do
    encode_tool_result_envelope(%{
      ok: false,
      error:
        SignalHelpers.error_envelope(:invalid_result, "Invalid tool result envelope", %{
  ...
  defp execute_internal(module, tool_name, params, context, timeout, exec_opts) do
    ...
    exec_opts_with_log =
      Keyword.put_new(exec_opts, :log_level, Jido.Observe.Config.action_log_level())
  ...
  defp format_error(tool_name, reason) when is_exception(reason) do
    SignalHelpers.normalize_error(reason, :execution_error, Exception.message(reason), %{
      tool_name: tool_name
    })
  end
