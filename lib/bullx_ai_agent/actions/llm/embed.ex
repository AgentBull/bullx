defmodule BullXAIAgent.Actions.LLM.Embed do
  @moduledoc """
  ...
  * `timeout` (optional) - Request timeout in milliseconds
  ...
  ## Examples
      ...
      {:ok, result} = Jido.Exec.run(BullXAIAgent.Actions.LLM.Embed, %{
        texts: "Hello world"
      })
  ...
  * `{:error, reason}` - Error from ReqLLM or validation
  ...
  defp calculate_total_length(texts),
    do: Enum.reduce(texts, 0, fn t, acc -> acc + String.length(t) end)
