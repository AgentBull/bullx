defmodule BullXAIAgent.Actions.Reasoning.Infer do
  @moduledoc """
  ...
  * `timeout` (optional) - Request timeout in milliseconds
  ...
  ## Examples
      ...
      # Basic inference
      {:ok, result} = Jido.Exec.run(BullXAIAgent.Actions.Reasoning.Infer, %{
  ...
  * `{:error, reason}` - Error from ReqLLM or validation
  ...
  defp validate_and_sanitize_params(params) do
    with {:ok, _premises} <-
           Validation.validate_string(params[:premises],
             max_length: Validation.max_input_length()
           ),
         {:ok, _question} <-
           Validation.validate_string(params[:question],
             max_length: Validation.max_input_length()
           ),
