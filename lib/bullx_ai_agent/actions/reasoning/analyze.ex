defmodule BullXAIAgent.Actions.Reasoning.Analyze do
  @moduledoc """
  ...
  * `timeout` (optional) - Request timeout in milliseconds
  ...
  ## Examples
      ...
      # Sentiment analysis
      {:ok, result} = Jido.Exec.run(BullXAIAgent.Actions.Reasoning.Analyze, %{
      ...
      Zoi.object(%{
        ...
        analysis_type:
          Zoi.enum([:sentiment, :topics, :entities, :summary, :custom],
            description:
              "Type of analysis to perform (:sentiment, :topics, :entities, :summary, :custom)"
          )
          |> Zoi.default(:summary),
        custom_prompt:
  ...
  * `{:error, reason}` - Error from ReqLLM or validation
  ...
  defp validate_and_sanitize_params(params) do
    with {:ok, _input} <-
           Validation.validate_string(params[:input], max_length: Validation.max_input_length()),
