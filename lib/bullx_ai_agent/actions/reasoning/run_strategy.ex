defmodule BullXAIAgent.Actions.Reasoning.RunStrategy do
  @moduledoc """
  ...
  alias BullXAIAgent.Actions.Helpers
  alias Jido

  alias BullXAIAgent.Actions.Reasoning.Runner.{
    AdaptiveAgent,
  ...
  @runner_jido BullXAIAgent.InternalReasoningRunner
  ...
  defp invoke_runner(runner, :cod, pid, prompt, timeout),
    do: runner.draft_sync(pid, prompt, timeout: timeout)

  defp invoke_runner(runner, :cot, pid, prompt, timeout),
    do: runner.think_sync(pid, prompt, timeout: timeout)

  defp invoke_runner(runner, :tot, pid, prompt, timeout),
    do: runner.explore_sync(pid, prompt, timeout: timeout)

  defp invoke_runner(runner, :got, pid, prompt, timeout),
    do: runner.explore_sync(pid, prompt, timeout: timeout)

  defp invoke_runner(runner, :trm, pid, prompt, timeout),
    do: runner.reason_sync(pid, prompt, timeout: timeout)

  defp invoke_runner(runner, :aot, pid, prompt, timeout),
    do: runner.explore_sync(pid, prompt, timeout: timeout)

  defp invoke_runner(runner, :adaptive, pid, prompt, timeout),
    do: runner.ask_sync(pid, prompt, timeout: timeout)
