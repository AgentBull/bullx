defmodule BullXAIAgent.Actions.ToolCalling.CallWithTools do
  @moduledoc """
  ...
  * `max_turns` (optional) - Max conversation turns with tools (default: `10`)
  ...
  ## Examples
      ...
      # Basic tool call
      {:ok, result} = Jido.Exec.run(BullXAIAgent.Actions.ToolCalling.CallWithTools, %{
        prompt: "What's 5 + 3?",
        tools: ["calculator"]
      })
  ...
  alias BullXAIAgent.{ToolAdapter, Turn, Validation}
  ...
  defp classify_and_format_response(response, model),
    do: Turn.from_response(response, model: model)
  ...
  defp execute_tool_turns(
    ...
    if turn_count > max_turns do
      ...
      case execute_tools_and_continue(
        ...
        {:error, reason} ->
          {:ok,
           %{type: :error, reason: reason, turns: turn_count, model: model, usage: usage_acc}}
      end
    end
  end
  ...
  defp validate_system_prompt_if_needed(%{system_prompt: system_prompt})
  ...
  defp append_assistant_message(messages, %Turn{} = turn),
    do: messages ++ [Turn.assistant_message(turn)]
