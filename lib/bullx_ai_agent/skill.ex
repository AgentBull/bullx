defmodule BullXAIAgent.Skill do
  @moduledoc """
  Unified skill abstraction for Jido agents.

  Skills can be defined two ways:

  1. **Compile-time modules** using `use BullXAIAgent.Skill`
  ...
  Load SKILL.md files at runtime:
      ...
      {:ok, spec} = BullXAIAgent.Skill.Loader.load("priv/skills/code-review/SKILL.md")
      BullXAIAgent.Skill.Registry.register(spec)

  ## Unified API

  Both types support the same interface:

      BullXAIAgent.Skill.manifest(skill)      # Returns the Spec struct
      BullXAIAgent.Skill.body(skill)          # Returns the skill body text
      BullXAIAgent.Skill.allowed_tools(skill) # Returns list of allowed tool names
      BullXAIAgent.Skill.actions(skill)       # Returns list of action modules
  """

  alias BullXAIAgent.Skill.{Spec, Registry, Error}
  ...
  defmacro __using__(opts) do
    quote bind_quoted: [opts: opts] do
      ...
      allowed_tools =
        BullXAIAgent.Skill.__normalize_allowed_tools__(Keyword.get(opts, :allowed_tools, []))
      ...
      BullXAIAgent.Skill.__validate_name__!(name)
      BullXAIAgent.Skill.__validate_description__!(description)

      body_ref = BullXAIAgent.Skill.__body_ref__(body, body_file)

      @skill_spec %BullXAIAgent.Skill.Spec{
        name: name,
      ...
      @impl BullXAIAgent.Skill
      def manifest, do: @skill_spec

      @impl BullXAIAgent.Skill
      def body, do: BullXAIAgent.Skill.__get_body__(@skill_spec.body_ref)
  ...
  def __normalize_allowed_tools__(tools) when is_binary(tools),
